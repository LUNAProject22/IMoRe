...
class NSTrajDatasetUnwrapped(FilterableDatasetUnwrapped):
    ...
    def __getitem__(self, index):
        ...
        # Extract the question type from the program's last function
        last_function = metainfo.program_raw[-1]["function"]
        feed_dict.last_function = last_function # query type
        ...
        if self.downsample:
            print("do downsample!!")
            raw_joints = self.sample_motion_sequence(raw_joints, sample_factor=4)
        ...
        m_length = motion_patches.shape[0]

        motion_patches = torch.tensor(motion_patches).float()
        motion_patches = rearrange(motion_patches, "t j c -> c t j")
