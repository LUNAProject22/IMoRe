...
parser.add_argument(
    "--no_gt_segments", action="store_true", help="dont use gt action segments"
)
parser.add_argument(
    "--temporal_operator", default="linear", choices=["linear", "conv1d", "shift"]
)
parser.add_argument(
    "--num-frames-per-seg",
    default=45,
    type=int,
    metavar="N",
    help="number of frames per each segment",
)
parser.add_argument(
    "--overlapping-frames",
    default=15,
    type=int,
    metavar="N",
    help="number of frames that each segment overlaps with one another",
)
...
parser.add_argument(
    "--evaluate",
    action="store_true",
    help="run the validation only; used with --load or --resume",
)
...
parser.add_argument(
    "--lr", type=float, default=1e-6, metavar="N", help="initial learning rate"
)  # default - 0.001
parser.add_argument(
    "--iters-per-epoch",
    type=int,
...
# parser.add_argument('--clip_grad', type=bool, default=True, help='gradient clipping')
parser.add_argument(
    "--validation-interval",
    type=int,
    default=1,
...
parser.add_argument(
    "--data_split_file",
    type="checked_file",
    default="./BABEL-QA/split_question_ids.json", # update this with dataset path
    metavar="FILE",
    help="file that contains which questions are included in train, val, and test",
)
...
parser.add_argument(
    "--use-tb", type="bool", default=True, metavar="B", help="use tensorboard or not"
)
parser.add_argument(
...
parser.add_argument("--featurefusion", default=True, type="bool", help="fuse intermediate features of Motion Patches encoder")
parser.add_argument("--downsample", default=1, type=int, help="downsample number of frames in motion sequence")

args = parser.parse_args()

# initialize wandb account
run = wandb.init(
    # entity='chinthani-team-org',
    entity='chinthani-team',
    project="NSPose",
    name="IMoRe II",
    config={
...
if args.use_gpu:
    ...
    print("args.gpu_parallel :", nr_devs)
...
def main():
    ...
    args.dump_dir = ensure_path(
        osp.join("dumps", args.series_name, args.desc_name, args.expr, args.run_name)
    )
    ...
    train_dataset = build_human_motion_dataset(
        args.data_dir,
        args.data_split_file,
        "train",
        downsample=args.downsample,
        no_gt_segments=args.no_gt_segments,
        num_frames_per_seg=args.num_frames_per_seg,
        overlapping_frames=args.overlapping_frames,
    )
    val_dataset = build_human_motion_dataset(
        args.data_dir,
        args.data_split_file,
        "val",
        downsample=args.downsample,
    ...
    test_dataset = build_human_motion_dataset(
        args.data_dir,
        args.data_split_file,
        "test",
        downsample=args.downsample,
...
def build_human_motion_dataset(
    data_dir,
    data_split_file,
    split,
    downsample,
    no_gt_segments=False,
    num_frames_per_seg=45,
    overlapping_frames=15,
    
):
    from datasets.humanmotionqa.dataset import NSTrajDataset

    dataset = NSTrajDataset(
        data_dir,
        data_split_file,
        split,
        downsample,
...
def main_train(train_dataset, validation_dataset, test_dataset):
    ...
    if hasattr(desc, "make_optimizer"):
        print("args.customized optimizer...")
    ...
    if args.clip_grad:
        print("gradient clipping activated!")
            ...
            for si, s in enumerate(curriculum_strategy):
                if curriculum_strategy[si][0] < epoch <= curriculum_strategy[si + 1][0]:
                    ...
                    if len(allowed_query_types) != 0:
                        this_train_dataset = this_train_dataset.filter_questions(
                            allowed_query_types
                        )
...
def train_epoch(epoch, trainer, train_dataloader, meters):
    ...
    with tqdm_pbar(total=nr_iters) as pbar:
        for i in range(nr_iters):
            ...
            run.log({"acc/qa": monitors['acc/qa'], "loss": loss})
